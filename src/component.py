...
class Component(ComponentBase):
    ...
    def build_table_mapping(self) -> list[dict]:
            ...
            for column in self.params.items:
                tbl.columns.append(
                    Column(
                        ...
                        convert_empty_values_to_null=column.nullable,  # design decision: same as "nullable"
                    )
                )
        ...
        if self.params.clone:
            in_table[0].pop("columns", None)  # columns not supported by API when using clone
        else:
